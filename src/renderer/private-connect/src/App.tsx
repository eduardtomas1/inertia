...
export default function App({ initialPairingFragment }: { initialPairingFragment: string | null }): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const load = async (): Promise<void> => {
      const response = await apiRequest({ protocolVersion: 1, type: "state.get", requestId: crypto.randomUUID() }, pair.csrf);
  ...
  useEffect(() => {
    ...
    void apiRequest({ protocolVersion: 1, type: "conversation.get", requestId: crypto.randomUUID(), conversationId: selectedConversation }, pair.csrf).then((response) => {
  ...
  }, [pair, selectedConversation]);

  const sendPrompt = async (): Promise<void> => {
    if (pair.kind !== "ready" || !prompt.trim() || !shell?.capabilities.scopes.includes("private:prompt") || !selectedConversation) return;
    setBusy(true);
    try {
      await apiRequest({ protocolVersion: 1, type: "prompt.send", requestId: crypto.randomUUID(), deliveryId: crypto.randomUUID(), conversationId: selectedConversation, content: prompt.trim() }, pair.csrf);
      setPrompt("");
  ...
  const answerQuestions = async (): Promise<void> => {
    ...
    setBusy(true);
    try {
      await apiRequest({ protocolVersion: 1, type: "input.respond", requestId: crypto.randomUUID(), conversationId: selectedConversation, inputRequestId: detail.inputRequestId, answers }, pair.csrf);
      setAnswers({});
    } finally { setBusy(false); }
  };
