...
import { MESSAGE_SEARCH_QUERY_MAX, type MessageSearchHit } from "@shared/message-search";
...
export function CommandPalette({ open, projects, conversations, newThreadShortcut, onClose, onSelectProject, onSelectConversation, sendCommand, onSelectMessage, onNewThread, onAddProject, onOpenSettings }: CommandPaletteProps): React.JSX.Element | null {
  ...
  const groups = (["Actions", "Projects", "Threads", "Messages"] as const).map((group) => ({ group, items: items.map((item, index) => ({ item, index })).filter(({ item }) => item.group === group) })).filter(({ items: groupItems }) => groupItems.length > 0);
    ...
    <div className="palette-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) closePalette(); }}>
      ...
      >
        ...
        <div className="palette-results" id="palette-results" role="listbox">
            ...
            <div className="palette-group" key={group}>
              ...
              {groupItems.map(({ item, index }) => (
                <button type="button" id={`palette-${item.id}`} role="option" aria-selected={activeIndex === index} className={activeIndex === index ? "is-active" : undefined} key={item.id} onPointerMove={() => setActiveIndex(index)} onClick={() => run(item)}>
                  {item.icon}<span><strong>{item.label}</strong>{item.detail && <small>{item.detail}</small>}{item.match && <span className="palette-message-snippet">{item.match.snippet.slice(0, item.match.matchStart)}<mark>{item.match.snippet.slice(item.match.matchStart, item.match.matchEnd)}</mark>{item.match.snippet.slice(item.match.matchEnd)}</span>}</span>{item.shortcut && <kbd>{item.shortcut}</kbd>}
                </button>
              ))}
            </div>
          ))}
          {search.loading && <div className="palette-search-status" role="status">Searching messages…</div>}
          {search.error && <div className="palette-search-status" role="status">{search.error}</div>}
          ...
          {search.result?.hasMore && <div className="palette-search-status" role="status">Showing the first 20 message matches. Refine your search to find more.</div>}
