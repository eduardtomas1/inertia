...
export class BoundedKimiJsonLineTransform extends Transform {
  ...
  private appendFragment(fragment: Buffer): void {
    ...
    if (nextBytes > this.maxLineBytes) {
      throw new Error("Kimi ACP sent an oversized JSON-RPC frame.");
    }
...
export function kimiSpawnFailure(
  error: unknown,
  workspaceRoot: string,
): ProviderRunFailure {
  const detail = kimiErrorDetail(error, "Kimi ACP could not be started.");
  ...
  return {
    reason: "process-exit",
    message: "Kimi ACP could not be started.",
    phase: "spawn",
...
export function kimiStopFailure(stopReason: string): ProviderRunFailure {
...
export interface KimiRuntimeFailureContext {
  child: ChildProcessWithoutNullStreams;
  processError?: Error;
  wireError?: Error;
  diagnostic: string;
...
export function kimiRuntimeFailure(
  error: unknown,
  context: KimiRuntimeFailureContext,
): ProviderRunFailure {
  const detail = [
    kimiErrorDetail(error, "Kimi ACP stopped unexpectedly."),
    context.wireError
      ? kimiErrorDetail(context.wireError, "Kimi ACP wire error.")
      : "",
    context.processError
      ? kimiErrorDetail(context.processError, "Kimi ACP process error.")
      : "",
    context.diagnostic,
  ].filter(Boolean).join("\n");
  ...
  const reason: ProviderRunFailure["reason"] = unsupportedTerminalAuth
    ...
    : /oversized|bounded (?:approval|input|tool activity|event|protocol)|safety limit|more than \d+ input options/iu.test(detail)
      ? "protocol-overflow"
      : /malformed|unexpected token|json-rpc frame|protocol version|not kimi code|invalid .* identity|invalid utf|not valid.*utf-?8|did not return a session id/iu.test(detail)
        ? "malformed-protocol"
  ...
  const message = unsupportedTerminalAuth
    ...
    : reason === "protocol-overflow"
      ...
      : reason === "malformed-protocol"
        ? "Kimi ACP returned a malformed protocol message."
        : reason === "rpc-timeout"
          ? "Kimi ACP stopped responding."
          : reason === "transport-closed"
            ? "The Kimi ACP connection closed before the turn completed."
            : reason === "process-signal" || reason === "process-exit"
              ? "Kimi ACP stopped before the turn completed."
              : sanitizeProviderFailureSummary(
                  detail,
...
export function kimiErrorDetail(error: unknown, fallback: string): string {
  const parts: string[] = [];
