...
describe("installed Claude SDK owned transport", () => {
  ...
  it.each([
    ...
    const run = harness.start({
      input: nativeProviderRunInput({ providerId: "claude", conversationId: "stderr-credentials", cwd: root,
        prompt: "Synthetic request", interactionMode: "build", access: "supervised" }),
      executable: process.execPath,
      environment: { ANTHROPIC_AUTH_TOKEN: "opaque-launch-credential-value" },
