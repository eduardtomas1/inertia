    ...
    if (request.url?.startsWith("/agent-browser-type-destination")) {
      setTimeout(() => {
        ...
        response.end(
          ...
          + "<script>console.error('password=browser-e2e-console-sentinel');console.error('pwd=hunter2');fetch('/agent-browser-evidence-failure?access_token=browser-e2e-query-sentinel',{method:'POST',body:'browser-e2e-body-sentinel'}).catch(()=>{});document.querySelector('[aria-label=\"Focus navigation\"]')"
