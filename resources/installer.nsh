  ...
  inertia_check_install_root:
    ${if} $IsPowerShellAvailable == 0
      ...
      ${if} $0 == 0
        StrCpy $R0 2
      ${else}
        nsExec::Exec `"$PowerShellPath" -NoLogo -NoProfile -NonInteractive -Command "try { $$rootPath = [IO.Path]::GetFullPath($$env:INERTIA_NSIS_INSTALL_ROOT).TrimEnd([char[]]'\/'); $$rootItem = Get-Item -LiteralPath $$rootPath -Force -ErrorAction Stop; if ($$rootItem -isnot [IO.DirectoryInfo] -or ($$rootItem.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0) { throw 'unsafe install root' }; $$root = [IO.Path]::GetFullPath($$rootItem.FullName).TrimEnd([char[]]'\/'); $$prefix = $$root + [IO.Path]::DirectorySeparatorChar; $$match = Get-CimInstance -ClassName Win32_Process -ErrorAction Stop | Where-Object { $$rawPath = [string]$$_.ExecutablePath; if ([String]::IsNullOrEmpty($$rawPath) -or -not $$rawPath.StartsWith($$root, [StringComparison]::OrdinalIgnoreCase)) { return $$false }; return [IO.Path]::GetFullPath($$rawPath).StartsWith($$prefix, [StringComparison]::OrdinalIgnoreCase) } | Select-Object -First 1; if ($$null -eq $$match) { exit 1 }; exit 0 } catch { exit 2 }"`
        Pop $R0
