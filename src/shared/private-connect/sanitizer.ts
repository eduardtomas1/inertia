...
const PRIVATE_CONNECT_VOID_HTML_TAGS = new Set([
  "area", "base", "br", "col", "embed", "hr", "img", "input", "link", "meta",
  "param", "source", "track", "wbr",
]);

interface PrivateConnectLine {
  contentEnd: number;
  nextStart: number;
}

interface PrivateConnectCodeFence {
  character: "`" | "~";
  length: number;
}

interface PrivateConnectHtmlTag {
  name: string;
  closing: boolean;
  complete: boolean;
  selfClosing: boolean;
...
interface PrivateConnectHtmlOpening {
  name: string;
  start: number;
  likelyHtml: boolean;
}

function privateConnectOutputLimit(maximumCharacters: number): number {
  return Math.min(
    Math.max(0, maximumCharacters),
    MAX_PRIVATE_CONNECT_CONTENT_CHARACTERS,
  );
}

export function privateConnectSanitizerInspectionWindow(
  value: string,
  maximumCharacters = MAX_PRIVATE_CONNECT_CONTENT_CHARACTERS,
): string {
  return value.slice(
    0,
    privateConnectOutputLimit(maximumCharacters) + SECRET_SCAN_MARGIN_CHARACTERS,
  );
}
...
function redactPrivateConnectCodeBlocks(value: string): string {
  ...
  while (lineStart < value.length) {
    const line = privateConnectLineAt(value, lineStart);
    const fence = privateConnectCodeFenceAt(value, lineStart, line.contentEnd);
    if (fence) {
      const redactedEnd = privateConnectFenceBlockEnd(
        value,
        line.nextStart,
        fence,
      );
    ...
    if (isIndentedPrivateConnectCodeLine(value, lineStart, line.contentEnd)) {
      const redactedEnd = privateConnectIndentedBlockEnd(value, line);
      result += value.slice(copiedUntil, lineStart);
      result += CODE_OMISSION;
      copiedUntil = redactedEnd;
      lineStart = redactedEnd;
...
function privateConnectFenceBlockEnd(
  value: string,
  start: number,
  fence: PrivateConnectCodeFence,
): number {
  let lineStart = start;
  while (lineStart < value.length) {
    const line = privateConnectLineAt(value, lineStart);
    if (isPrivateConnectFenceClose(
      value,
      lineStart,
      line.contentEnd,
      fence,
...
function privateConnectIndentedBlockEnd(
  value: string,
  firstLine: PrivateConnectLine,
): number {
  ...
  while (lineStart < value.length) {
    ...
    if (isBlankPrivateConnectLine(value, lineStart, line.contentEnd)) {
      let nextContentStart = line.nextStart;
      while (nextContentStart < value.length) {
        const next = privateConnectLineAt(value, nextContentStart);
        if (!isBlankPrivateConnectLine(
          value,
          nextContentStart,
          next.contentEnd,
        )) break;
      ...
      if (nextContentStart < value.length) {
        const next = privateConnectLineAt(value, nextContentStart);
        if (isIndentedPrivateConnectCodeLine(
          value,
          nextContentStart,
          next.contentEnd,
        )) {
...
function privateConnectCodeFenceAt(
  value: string,
  start: number,
  end: number,
): PrivateConnectCodeFence | null {
  const markerStart = privateConnectFenceMarkerStart(value, start, end);
  if (markerStart === null) return null;
  const character = value[markerStart];
  if (character !== "`" && character !== "~") return null;
  const markerEnd = scanPrivateConnectFenceRun(value, markerStart, end, character);
  const length = markerEnd - markerStart;
  if (length < 3) return null;
  return { character, length };
}

function isPrivateConnectFenceClose(
  value: string,
  start: number,
  end: number,
  fence: PrivateConnectCodeFence,
  ...
  const markerEnd = scanPrivateConnectFenceRun(
    value,
    markerStart,
    end,
    fence.character,
...
function privateConnectContainerContentStart(
  value: string,
  start: number,
  end: number,
): number {
  ...
  return privateConnectListMarkerEnd(value, cursor, end) ?? cursor;
}

function privateConnectListMarkerEnd(
  value: string,
  start: number,
  end: number,
): number | null {
...
function privateConnectFenceMarkerStart(
  value: string,
  start: number,
  end: number,
): number | null {
  const contentStart = privateConnectContainerContentStart(value, start, end);
...
function scanPrivateConnectFenceRun(
  value: string,
  start: number,
  end: number,
  character: "`" | "~",
...
function isIndentedPrivateConnectCodeLine(
  value: string,
  start: number,
  end: number,
): boolean {
  if (isBlankPrivateConnectLine(value, start, end)) return false;
  const contentStart = privateConnectContainerContentStart(value, start, end);
  if (isBlankPrivateConnectLine(value, contentStart, end)) return false;
...
function isBlankPrivateConnectLine(
  value: string,
  start: number,
  end: number,
): boolean {
...
function privateConnectLineAt(value: string, start: number): PrivateConnectLine {
...
function redactPrivateConnectHtmlBlocks(value: string): string {
  ...
  const openings: PrivateConnectHtmlOpening[] = [];
  let index = 0;
  while (index < value.length) {
    ...
    const specialEnd = privateConnectSpecialHtmlEndAt(value, index);
    if (specialEnd !== null) {
      redactionEnds[index] = specialEnd;
      index = specialEnd;
      continue;
    }
    const tag = privateConnectHtmlTagAt(value, index);
    if (!tag) {
      index += 1;
      continue;
    }
    const known = PRIVATE_CONNECT_HTML_TAGS.has(tag.name);
    ...
    if (tag.selfClosing || PRIVATE_CONNECT_VOID_HTML_TAGS.has(tag.name)) {
...
function privateConnectSpecialHtmlEndAt(
  value: string,
  start: number,
): number | null {
...
function privateConnectHtmlTagAt(
  value: string,
  start: number,
): PrivateConnectHtmlTag | null {
  let cursor = start + 1;
  ...
  while (cursor < value.length) {
    ...
    if (character === ">") {
      ...
      return {
        ...
        hasAttributes: hasPrivateConnectHtmlAttributes(
          value,
          attributeStart,
          cursor,
        ),
...
function hasPrivateConnectHtmlAttributes(
  value: string,
  start: number,
  end: number,
): boolean {
...
export function sanitizePrivateConnectLabel(
  value: string | null | undefined,
  maximumCharacters = 240,
): string | null {
  if (typeof value !== "string") return null;
  const text = sanitizePrivateConnectContent(
