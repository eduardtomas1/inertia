...
import {
  PRIVATE_CONNECT_TRANSCRIPT_CACHE_BUDGET_BYTES,
  PrivateConnectTranscriptCache,
  privateConnectTranscriptFingerprint,
} from "../../../src/server/private-connect/transcript-cache";
import { sanitizePrivateConnectContent } from "../../../src/shared/private-connect/sanitizer";
...
describe("Private Connect transcript cache", () => {
  it("keeps retained memory under the budget for hundreds of huge messages", () => {
    const cache = new PrivateConnectTranscriptCache({
      fingerprint: () => "shared",
  ...
  it("returns the correct sanitized content on a cache hit", () => {
    ...
    const cache = new PrivateConnectTranscriptCache({
      sanitize: (source) => {
        sanitizeCalls += 1;
        return sanitizePrivateConnectContent(source);
      },
    });
    ...
    expect(first).toBe(sanitizePrivateConnectContent(source));
  ...
  it("fingerprints only the bounded window the sanitizer inspects", () => {
    ...
    expect(privateConnectTranscriptFingerprint(`${prefix}beyond-window-one`))
      .toBe(privateConnectTranscriptFingerprint(`${prefix}beyond-window-two`));
    expect(privateConnectTranscriptFingerprint("short"))
      .not.toBe(privateConnectTranscriptFingerprint("shorter"));
  });

  it("does not let a shorter window collide with a longer one", () => {
    expect(privateConnectTranscriptFingerprint("12ab"))
      .not.toBe(privateConnectTranscriptFingerprint("2ab"));
  });
  ...
  it("never retains an entry larger than the whole budget", () => {
    const cache = new PrivateConnectTranscriptCache({ budgetBytes: 128 });
  ...
  it("keeps the byte total consistent across mixed operations", () => {
    const cache = new PrivateConnectTranscriptCache({ budgetBytes: 100_000 });
