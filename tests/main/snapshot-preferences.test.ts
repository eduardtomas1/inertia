...
import { clearSnapshotPreferences, readSnapshotPreferences, writeSnapshotPreferences } from "../../src/main/snapshot-preferences";
...
it("forgets saved preferences so the next launch starts disabled, and tolerates absence", async () => {
  const root = await directory();
  await writeSnapshotPreferences(root, { enabled: true, shortcut: "both-shift" });
  await clearSnapshotPreferences(root);
  expect(await readSnapshotPreferences(root)).toBeNull();
  expect(await readdir(root)).toEqual([]);
  await expect(clearSnapshotPreferences(root)).resolves.toBeUndefined();
});
