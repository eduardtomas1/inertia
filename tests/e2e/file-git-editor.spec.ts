...
test("Git badges reflect real status and colored editing preserves text, scrolling, undo and guarded saves", async ({ browserName: _browserName }, testInfo) => {
  ...
  try {
    ...
    await expect(panel.getByRole("treeitem", { name: /sample.ts Git: Modified/u })).toBeVisible();
    const added = panel.getByRole("treeitem", { name: /Added.java Git: Added · staged/u });
    await expect(added).toBeVisible();
    await expect(panel.getByRole("treeitem", { name: /untracked.txt Git: Untracked/u })).toBeVisible();
    ...
    await expect(panel.getByRole("treeitem", { name: /Added.java Git: Added · staged and unstaged/u })).toBeVisible();
    ...
    await expect(panel.getByRole("treeitem", { name: "sample.ts", exact: true })).toBeVisible();
