...
describe("Private Connect safe text projection", () => {
  it("removes local paths, credential material, code, and HTML blocks", () => {
    const projected = sanitizePrivateConnectContent([
  ...
  it("redacts code inside blockquote and list containers", () => {
    ...
    for (const [input, expected] of cases) {
      expect(sanitizePrivateConnectContent(input)).toBe(expected);
    }
    expect(sanitizePrivateConnectContent("<?php echo $secret; ?>"))
      .toBe("[HTML omitted on Private Connect]");
  });
  ...
  it("keeps ordinary quoted prose and list items readable", () => {
    ...
    for (const input of cases) {
      expect(sanitizePrivateConnectContent(input)).toBe(input);
    }
  });

  it("redacts url user-info through the authority's final at-sign", () => {
    expect(sanitizePrivateConnectContent("https://alice:pa@ss@example.com/private"))
      .toBe("https://<redacted>@example.com/private");
    expect(sanitizePrivateConnectContent("https://a:b@c@d@example.com/x"))
      .toBe("https://<redacted>@example.com/x");
    expect(sanitizePrivateConnectContent("see https://example.com/a@b for details"))
      .toBe("see https://example.com/a@b for details");
    expect(sanitizePrivateConnectContent("https://example.com/?q=a@b"))
      .toBe("https://example.com/?q=a@b");
    expect(sanitizePrivateConnectContent("mail bob@example.com now"))
  ...
  it("redacts alternate, interrupted, and indented code blocks", () => {
    const cases = [
      [
        ...
        "Before\n[Code omitted on Private Connect]\nAfter",
      ],
      [
    ...
    for (const [input, expected] of cases) {
      expect(sanitizePrivateConnectContent(input)).toBe(expected);
      expect(sanitizePrivateConnectContent(input)).not.toContain("Secret");
    }
    expect(sanitizePrivateConnectContent(
  ...
  it("redacts interrupted, self-closing, and nested HTML linearly", () => {
    const cases = [
      [
        ...
        "Before [HTML omitted on Private Connect] after",
      ],
      [
      ...
      [
        ...
        "Before [HTML omitted on Private Connect] unmatched prose after",
      ],
      [
        "Before <div><span>mismatched secret</div> after",
        "Before [HTML omitted on Private Connect]",
      ],
      [
  ...
  it("redacts POSIX absolute tokens with legal punctuation and separator runs", () => {
    ...
    for (const path of paths) {
      ...
      expect(sanitizePrivateConnectContent(`Path (${path}).`)).toBe(
        "Path (<local-path>).",
      );
      expect(sanitizePrivateConnectContent(`Path label:${path}, then done.`)).toBe(
        "Path label:<local-path>, then done.",
      );
      expect(sanitizePrivateConnectContent(`Path ${path}`)).not.toContain(path);
    }
    for (const path of ["/", "/.", "/..", "/!"]) {
      expect(sanitizePrivateConnectContent(`Path ${path}`)).toBe(
        "Path <local-path>",
  ...
  it("preserves URLs, relative paths, and ordinary escaped prose", () => {
    ...
    expect(sanitizePrivateConnectContent(ordinaryText)).toBe(ordinaryText);
  });
  ...
  it("handles surrounding delimiters without treating web URLs as paths", () => {
    ...
    for (const url of [
      ...
      expect(sanitizePrivateConnectContent(url)).toBe(url);
    }
  });
  ...
  it("bounds and normalizes labels", () => {
    ...
    expect(sanitizePrivateConnectContent(adversarialBoundaries))
    ...
    expect(sanitizePrivateConnectContent(regexToken)).toBe(regexToken);
    ...
    expect(sanitizePrivateConnectContent(`${mixedToken}${mixedSuffix}`)).toBe(
    ...
    expect(sanitizePrivateConnectContent(interruptedFence)).toBe(
      "[Code omitted on Private Connect]",
    );
    ...
    expect(sanitizePrivateConnectContent(angleProse)).toBe(angleProse);
    expect(performance.now() - htmlStartedAt).toBeLessThan(2_000);
    expect(sanitizePrivateConnectLabel("  hello\u0000\n world ", 20)).toBe(
      "hello world",
    );
    expect(sanitizePrivateConnectLabel("x".repeat(100), 20)).toHaveLength(20);
    expect(sanitizePrivateConnectLabel(
      "\u202eTrusted\u202c\u0000 Browser\u2066",
      80,
