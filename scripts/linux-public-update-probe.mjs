...
import { validatePublicUpgrade } from "./linux-public-update-evidence.mjs";

if (process.platform !== "linux" || process.arch !== "x64") throw new Error("This public probe requires native Linux x64.");
assert([6, 7, 10].includes(process.argv.length));
const completeUpgrade = process.argv[6] === "public-upgrade";
if (process.argv.length > 6) assert(completeUpgrade);
if (completeUpgrade) validatePublicUpgrade(...process.argv.slice(4, 6), ...process.argv.slice(7));
else {
...
try {
  const output = await runLinuxGuardedSmoke({
    ...
    args: [resolve(import.meta.dirname, completeUpgrade ? "linux-public-upgrade-driver.mjs" : "linux-public-update-probe-driver.mjs"), resolve(process.argv[2]), root, process.argv[4], process.argv[5], ...process.argv.slice(7)],
  });
