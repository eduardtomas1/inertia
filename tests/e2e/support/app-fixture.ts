    ...
    if (request.url?.startsWith("/agent-browser-type-destination")) {
      setTimeout(() => {
        ...
        response.end(
          ...
          + "<script>console.error('password=browser-e2e-console-sentinel');console.error('pwd=hunter2');console.error('MONGODB_URI=mongodb://alice:hunter2@localhost/private');console.error('Failure at C://Users/Jane Doe/private/file.txt');console.error('Failure at //private-server/secret share/file.txt');console.error('Failed in src/private/config');console.error('Failed in src/.env');console.error('Failed in ./Dockerfile');fetch('/agent-browser-evidence-failure?access_token=browser-e2e-query-sentinel',{method:'POST',body:'browser-e2e-body-sentinel'}).catch(()=>{});document.querySelector('[aria-label=\"Focus navigation\"]')"
