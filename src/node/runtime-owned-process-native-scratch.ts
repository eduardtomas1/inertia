...
const errorNames = new Set(["Error", "GitError", "TypeError", "RangeError", "SyntaxError", "SqliteError", "RuntimeStartupBlockerError"]);
const errorCodes = new Set(["invalid-input", "not-repository", "not-found", "conflict", "nothing-to-commit", "authentication", "output-limit", "timeout", "git-unavailable", "operation-failed", "ENOENT", "EACCES", "EPERM", "EIO", "EMFILE", "ENFILE", "EEXIST", "ENOTDIR", "EADDRINUSE", "SQLITE_BUSY", "SQLITE_LOCKED", "SQLITE_ERROR", "SQLITE_CORRUPT", "SQLITE_CANTOPEN"]);

export function recordStartupScratch(phase: "enter" | "git-prewarm" | "generated-attachments" | "persistence" | "backend" | "listen" | "ready" | "rejected", error?: unknown, gitCleanupFailure = false): void {
  ...
  try {
    ...
    const name = typeof value?.name === "string" && errorNames.has(value.name) ? value.name : error === undefined ? null : "other";
    const code = typeof value?.code === "string" && errorCodes.has(value.code) ? value.code : null;
    let cwdDirectory = false;
    let cwdMatchesHome = false;
    try { const cwd = process.cwd(); cwdDirectory = lstatSync(cwd).isDirectory(); cwdMatchesHome = cwd === home; } catch { /* Boolean only. */ }
    const line = JSON.stringify({ event: "runtime-startup", at: Date.now(), phase, name, code, gitCleanupFailure, cwdDirectory, cwdMatchesHome }) + "\n";
    if (!stat.isFile() || (stat.mode & 0o777) !== 0o600 || stat.size + Buffer.byteLength(line) > 16_384) return;
    appendFileSync(path, line);
  } catch { /* Fixed synthetic diagnostics never affect startup. */ }
}
