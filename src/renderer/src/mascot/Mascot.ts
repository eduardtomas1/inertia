...
export function mountMascot(root: HTMLElement, bridge: MascotBridge): () => void {
  ...
  root.innerHTML = `<main class="mascot" tabindex="0">
    ...
    <button class="mascot-status" type="button" title="Open chat. Arrow keys move the mascot; Escape hides it.">
      <span class="mascot-label" role="status" aria-live="polite"></span>
      <span class="mascot-detail"></span>
    </button>
  </main>`;
  ...
  const render = (): void => {
    ...
    button.setAttribute("aria-label", `${label.textContent}${status.conversationId ? ". Open chat" : ""}`);
    detail.textContent = status.activeCount > 1 ? `${status.activeCount} active chats · Open`
      : status.conversationId ? "Open chat ↗" : "Inertia";
  };
