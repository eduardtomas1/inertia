...
- A removed CLI selected through PATH retained the old physical installation identity while probing its replacement. Lease settlement quarantined this legitimate relocation. A changed or unverified installation now discards capability evidence before resolving the configured command again, as at startup. Existing use leases and quarantine are retained; a replacement still needs successful discovery and protocol verification.
...
- Focused provider refresh, installation, lease and turn preparation coverage: 93 tests passed.
- `VITEST_MAX_WORKERS=4 npm run check`: passed; 9,263 tests passed, 145 platform/optional tests skipped. Quality checks and production build passed with unchanged renderer budgets.

- `npm run test:portable`: 1,392 passed and 9 platform/optional tests skipped across 101 files.
...
Final review also reproduced relocation after the old installation failed its first protocol check: it had a resolved command but no verified capability evidence. The same re-resolution is now applied to both changed and unverified installations. The parameterized regression covers both states, preserves prior use leases, and leaves quarantine enforcement intact.
