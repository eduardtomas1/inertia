...
export interface PromptStashRoute extends Pick<
  ModelSelection,
  "harnessId" | "backendProfileId" | "modelId" | "reasoningEffort"
> {
  /** Optional only for v1 stash entries written before Fast mode shipped. */
  fastMode?: boolean;
}
...
export function promptStashRouteMatches(
  selection: PromptStashRoute | ModelSelection,
  route: PromptStashRoute,
): boolean {
  if (route.fastMode !== undefined && !routeSupportsNativeFastModeIdentity(route)) {
    return false;
  }
  const selectedFastMode = "providerOptions" in selection
  ...
  return selection.harnessId === route.harnessId
    ...
    && selectedFastMode === (route.fastMode ?? false);
}

export function promptStashRestoreBlockedReason(
  selection: PromptStashRoute | ModelSelection,
  route: PromptStashRoute,
): string | null {
  if (promptStashRouteMatches(selection, route)) return null;
  const selectedFastMode = "providerOptions" in selection
    ? modelSelectionUsesFastMode(selection)
    : selection.fastMode === true;
  return `Use ${route.modelId} · ${
    route.reasoningEffort ?? "default reasoning"
  }${!routeSupportsNativeFastModeIdentity(route)
    || (route.fastMode === undefined && !selectedFastMode)
    ? ""
    : ` · ${route.fastMode ? "Fast" : "Standard"} speed`} to restore`;
}
...
function parsedRoute(value: unknown): PromptStashRoute | null {
  ...
  if (
    ...
    || (route.fastMode !== undefined && typeof route.fastMode !== "boolean")
    || (route.fastMode !== undefined && !routeSupportsNativeFastModeIdentity({
